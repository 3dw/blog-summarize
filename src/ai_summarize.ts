type SummarizeEnv = {
	AI: {
		run: (model: string, payload: { instructions: string; input: string }) => Promise<any>;
	};
};
...
async function mergeSummaries(summaries: string[]): Promise<string> {
	...
	return summaries
		.filter((summary) => summary.replace(/\s/g, "").length > 0)
		.map((summary, index) => `## 第${index + 1}部分\n${summary}`)
		.join("\n\n");
}

// 並行處理chunks的輔助函數
async function processChunks(chunks: string[], env: SummarizeEnv): Promise<string> {
	if (chunks.length === 1) {
		console.log("單段落處理");
	...
	// 並行處理所有chunks（提高效率）
	const summaryPromises = chunks.map((chunk, index) => generateChunkSummary(chunk, env, index, chunks.length));

	try {
		const summaries = await Promise.all(summaryPromises);
		console.log("所有段落處理完成，開始合併");
		const finalOutline = await mergeSummaries(summaries);
		console.log("大綱生成完成");
		return finalOutline;
	} catch (error) {
...
// 降級的逐一處理函數
async function processChunksSequentially(chunks: string[], env: SummarizeEnv): Promise<string> {
	console.log("降級為逐一處理");
	...
	return summaries.map((summary, index) => `## 第${index + 1}部分\n${summary}`).join("\n\n");
}

export async function generateOutline(transcription: string, env: SummarizeEnv): Promise<string> {
